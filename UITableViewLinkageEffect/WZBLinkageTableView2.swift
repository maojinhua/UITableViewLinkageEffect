...
class WZBLinkageTableView2: UIViewController,UITableViewDelegate,UITableViewDataSource {
    ...
    {
        //添加左边的Tableview
        var leftTableView = UITableView(frame: CGRectMake(0, 0, WZBScreenWidth * 0.25, WZBScreenHeight))
        leftTableView.backgroundColor = UIColor.redColor();
        self.view.addSubview(leftTableView);
        self.leftTableView = leftTableView;
        //添加右边的TableView
        var rightTableView = UITableView(frame: CGRectMake(WZBScreenWidth * 0.25, 0, WZBScreenWidth * 0.75, WZBScreenHeight))
    ...
    func selectLeftTableViewWthScrollView(scrollView:UIScrollView)
    {
        ...
        if(scrollView as! UITableView == self.leftTableView)
        {return;}
    ...
    func tableView(tableView: UITableView, didSelectRowAtIndexPath indexPath: NSIndexPath) {
        ...
        {// 如果点击的是右边的tableView，不做任何处理
            ...
            var path:NSIndexPath = NSIndexPath(forRow: 0, inSection: indexPath.row);
